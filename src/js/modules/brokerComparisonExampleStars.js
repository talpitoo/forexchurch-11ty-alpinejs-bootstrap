...
export function brokerComparisonExample() {
  return {
    ...
    initTable() {
      this.table = new Tabulator("#comparison-table", {
        ...
        rowFormatter: function(row) {
          ...
          if (data.detail === "Account Types") {
            row.getCells().forEach(function(cell) {
              // Assuming you have preformatted your Account Types data
              // Or manually insert HTML for stars here
              if(cell.getField() !== "detail") {
                ...
                for(let i = 0; i < value; i++) {
                  starsHtml += '<span class="me-2">&#9733;</span>'; // Append star HTML
                }
                cell.getElement().innerHTML = starsHtml + cell.getValue(); // Set the cell's inner HTML to the stars
              }
            });
          }
        },
    ...
    loadSelectedBrokersData() {
      const fetchPromises = this.selectedBrokers.map(brokerId => axios.get(brokerDataUrls[brokerId]));

      Promise.all(fetchPromises).then(responses => {
        const brokersData = responses.map(response => response.data);
